...
public class IndexerVisualisation : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (m_ActiveScalerProfile != Holder.Instance.Settings.ActiveScalerProfile)
        {
            m_ActiveScalerProfile = Holder.Instance.Settings.ActiveScalerProfile;
            foreach (var scalerVisualisation in m_ScalerVisualisations)
            {
                Destroy(scalerVisualisation.gameObject);
            }
            
            m_ScalerVisualisations.Clear();
        }
